...
handsOn:
  online:
    platform: killercoda
    enabled: true
    url: "https://killercoda.com/egykode/scenario/incident-crashloopbackoff"
  local:
    enabled: true
    environment: k8s
    tools:
    ...
    capabilities:
      - kubernetes-cluster
successCriteria:
  - text: "The Pod reaches `Running` with `READY 1/1` and stays there for two minutes."
    verify: state
  - text: "You can state the exit code and what it meant, and name the mechanism that produced it."
    verify: reasoning
  - text: "You can explain why `kubectl logs` alone was not enough."
    verify: reasoning
authors: [waleed]
updated: 2026-08-10
---
